...
public class Level1ErrorController {
	...
	/**
	 * Business exceptions, managed by the application. That's why we don't display
	 * stack trace in INFO level
	 */
	@ExceptionHandler(value = { BusinessException.class, HttpMessageNotReadableException.class,
	...
	public ResponseEntity<StatusDTO> handleAccessDeniedException(AccessDeniedException e) {
		...
		String message = messageSource.getMessage("sitedia.commonRest.endpoint.accessDenied", null,
				Locale.getDefault());
	...
	/**
	 * Others exceptions. For security reasons, the message is not returned to the
	 * user
	 */
	...
	public ResponseEntity<StatusDTO> handleException(Exception e) {
		...
		String message = messageSource.getMessage("sitedia.commonRest.endpoint.internalError", null,
